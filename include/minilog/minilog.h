...
#define _MINILOG_IF_HAS_ANSI_COLORS(x)
    inline constexpr char
        k_level_ansi_colors[(std::uint8_t)log_level::fatal + 1][1] = {
            "", "", "", "", "", "", "",
    };
...
#undef _FUNCTION
    ...
    inline void output_log(
        log_level level, std::string msg, std::source_location const &loc) {
        ...
        if (level >= g_max_level) {
            std::cout << _MINILOG_IF_HAS_ANSI_COLORS(
                             k_level_ansi_colors[(std::uint8_t)level] +)
                                 msg _MINILOG_IF_HAS_ANSI_COLORS(
                                     +k_reset_ansi_color)
                             + '\n';
        }
    }
