...
struct Function<Ret(Args...)> {
  ...
  struct FunctionImpl : FunctionBase {
    ...
    virtual Ret call(Args... args) override {
      return std::invoke(m_f,
                         std::forward<Args>(args)...);  // 完美转发所有参数给构造时保存的仿函数对象
    }
  };
  ...
  Function(F f)
      : m_base(std::make_shared<FunctionImpl<F>>(
            std::move(f))){};  // 没有 explicit，允许 lambda 表达式隐式转换成 Function
